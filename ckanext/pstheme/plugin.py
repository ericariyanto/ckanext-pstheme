...
import ckanext.pstheme.cli as cli
# import ckanext.pstheme.helpers as helpers
# import ckanext.pstheme.views as views
# from ckanext.pstheme.logic import (
...
def get_visualization():
    return config.get('ckanext.pstheme.visualization', '')

def get_metadata_fields():
    metafields = config.get('ckanext.pstheme.metadata_fields', '')
    if not metafields:
        return []
...
class PsthemePlugin(plugins.SingletonPlugin, toolkit.DefaultDatasetForm):
    ...
    def update_config(self, config_):
        ...
        toolkit.add_resource("assets", "pstheme")
    ...
    def update_config_schema(self, schema):
        ...
        schema.update({
            'ckanext.pstheme.visualization': [ignore_missing, unicode_safe],
            'ckanext.pstheme.metadata_fields': [ignore_missing, unicode_safe],
        })
    ...
    def get_helpers(self):
        ...
        # other extensions.
        return {
            'pstheme_group_list': group_list,
            'pstheme_format_number' : format_number, 
            'pstheme_get_visualization': get_visualization,
            'pstheme_get_metadata_fields': get_metadata_fields,
            'pstheme_get_metadata_keys_fields': get_metadata_keys_fields,
            'pstheme_get_metadata_object': get_metadata_object,
            'pstheme_get_metadata_object_val': get_metadata_object_val,
        }
    ...
    def create_package_schema(self) -> Schema:
        ...
        schema: Schema = super(PsthemePlugin, self).create_package_schema()
        schema = self._modify_package_schema(schema)
        return schema

    def update_package_schema(self) -> Schema:
        self.write_log('update_package_schema')
        schema: Schema = super(PsthemePlugin, self).update_package_schema()
        schema = self._modify_package_schema(schema)
        return schema

    def show_package_schema(self) -> Schema:
        self.write_log('show_package_schema updated')
        schema: Schema = super(PsthemePlugin, self).show_package_schema()        
        # schema = self._modify_package_schema(schema)
